...
class SelfAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.view(-1, self.channels, self.size[0] * self.size[1]).swapaxes(1, 2).contiguous()
        x_ln = self.ln(x)
        # batch_first is not supported in pytorch 1.8.
        # If you want to support upgrading to 1.9 and above, or use the following code to transpose
        ...
        return attention_value.swapaxes(2, 1).view(-1, self.channels, self.size[0], self.size[1]).contiguous()
